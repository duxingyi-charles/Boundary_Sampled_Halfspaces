...
//    wFlip = para.wFlip;
    ...
    auto t2 = Clock::now();
    setup_time = std::chrono::nanoseconds(t2 - t1).count()/1e9;
#ifdef DEBUG
    cout << "Build Time: " << setup_time << endl<< endl;
#endif
...
void RBF_Core::InitNormal(RBF_Paras para){
    ...
    auto t2 = Clock::now();
    init_time = std::chrono::nanoseconds(t2 - t1).count()/1e9;
#ifdef DEBUG
    cout << "Init Time: " << init_time << endl<< endl;
#endif
...
void RBF_Core::OptNormal(int method){
    ...
    auto t2 = Clock::now();
    solve_time = std::chrono::nanoseconds(t2 - t1).count()/1e9;
#ifdef DEBUG
    cout << "Opt Time: " << solve_time << endl<< endl;
#endif
